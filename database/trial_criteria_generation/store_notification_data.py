...
from trial_criteria_generation.models.db_models import NotificationData
from typing import Dict, Any

# Initialize MongoDB Data Access Object (DAO)
mongo_dao = MongoDBDAO()

def store_notification_data(trialId: str) -> Dict[str, Any]:
    """
    ...
    Args:
        trialId (str): The Job ID associated with the Job Run
    ...
    Example:
        >>> store_notification_data("john_doe", "trialId_123", "Your order has been shipped.")
        {
            "success": True,
    ...
    try:
        # Fetch User Name
        user_name_response =  mongo_dao.find_one(collection_name="similar_trials_results", query={"trialId": trialId}, projection={"userName": 1})
        user_name = user_name_response["userName"] if user_name_response else "Unknown User"

        # Notification Message

        notification_message = f"Hey {user_name}, your job with Trial ID: {trialId} has been completed successfully."
