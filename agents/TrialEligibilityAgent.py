...
from trial_criteria_generation.utils.generate_metrics_prompt import generate_metrics_prompt
...
class TrialEligibilityAgent:
    ...
    def _construct_user_input(self, draft_criteria: DraftEligibilityCriteria) -> str:
        ...
        """
        logger.debug("Constructing user input")
        user_input = f"""
    ...
    def _generate_criteria_with_ai(self, user_input: str, system_prompt: str) -> tuple:
        ...
        try:
          ...
          if response["success"] is False:
            ...
            logger.exception(f"Error generating criteria with AI: {e}")
    ...
    def _extract_drug_metrics(self, similar_trial_documents: Dict) -> List:
        ...
        try:
            ...
            if match:
                ...
                logger.debug("Extracted Final response from Drug Metrics")
                return final_response
            else:
                return []
        except Exception as e:
            logger.exception(f"Error extracting drug metrics: {e}")
            logger.debug(metrics_response["data"])
            print("*"*100)
            return []
    ...
    def _extract_timeframe_metrics(self, similar_trial_documents: Dict) -> List:
        ...
        try:
            ...
            if timeline_response["success"] is False:
                logger.exception(timeline_response["message"])
                return []
            else:
                # Extract JSON
            ...
            logger.exception(f"Error extracting timeframe metrics: {e}")
            logger.debug(timeline_response["data"])
            print("*"*100)
            return []
